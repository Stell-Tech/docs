...
- **Pass status changed** — fires each time a pass moves between [lifecycle states](/passes/lifecycle). The most common use is detecting enrollment: when a customer adds their pass to Apple Wallet or Google Wallet, the pass goes from Pre-Active to Active, and that change fires the webhook.
