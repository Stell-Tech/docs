...
- Exception: `images/promote/` holds designed Mintleaf marketing mockups (email render, social posts, poster, counter card, POS slide) — not part of the Playwright portal pipeline. Sources are the HTML files in `mockups/`; regenerate with `node mockups/capture.mjs` (the wallet badge SVGs in `mockups/assets/` come from the Stell embed assets)
